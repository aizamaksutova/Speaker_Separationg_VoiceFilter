...
def train_new_embed(args, pt_dir, chkpt_path, trainloader, writer, logger, hp):
    # load embedder
    embedder = torch.hub.load('RF5/simple-speaker-embedding', 'convgru_embedder')
        ...
        for _ in range(num_epochs):
            ...
            for dvec_mels, target_wav, mixed_wav, target_mag, mixed_mag, mixed_phase in trainloader:
                ...
                for mel in dvec_mels:
                    mel = mel.unsqueeze(1).repeat(1, 2, 1).cuda()
                    print(mel.shape)
                    dvec = embedder(mel)
